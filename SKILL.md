...
温度分 = 市场状态(位置, 越高越热: 过热=贵/拥挤/亢奋)
    ...
    拥挤20%: CFTC净多分位8(热) + ETF份额增速分位7(热) + 波动率分位5(热)
    宏观20%: 美元指数分位8(冷) + VIX分位6(热) + 通胀预期分位6(热)
...
| ETF份额(沪)            | akshare `fund_etf_scale_sse(date)`                                                | 指定单日快照, 逐日落库累积                                 |
...
- **ETF份额增速分位缺失("份额累积中")** → 份额需本地逐日积累，冷启动约需 60 个运行日；首次运行可 `--backfill-shares 60` 加速。

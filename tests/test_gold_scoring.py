...
# ------------------------------------------------------------
# 15. 央行购金展示指标(纯函数, 不进分数)
# ------------------------------------------------------------
cb_none = G.cb_gold_metrics([], [], [], 4400.0)
check("央行: 无数据→None", cb_none is None)
_d = [f"2025-{m:02d}" for m in range(1, 13)] + [f"2026-{m:02d}" for m in range(1, 9)]
_w = [7280.0 + k * 8.0 for k in range(20)]            # 连涨20个月
cb = G.cb_gold_metrics(_d, _w, [34000.0] * 20, 4400.0)
check("央行: 吨换算(万盎司×0.31103)",
      approx(cb["tonnes"], _w[-1] * 0.311034768, 0.1), cb)
check("央行: 近12月净增",
      approx(cb["d12_tonne"], (_w[-1] - _w[-13]) * 0.311034768, 0.1), cb["d12_tonne"])
check("央行: 连续增持月数", cb["streak"] == 19, cb["streak"])
check("央行: 占外储比重",
      approx(cb["share_pct"], _w[-1] * 1e4 * 4400.0 / (34000.0 * 1e8) * 100, 0.01),
      cb["share_pct"])
_w2 = _w[:-1] + [_w[-2] - 1.0]                        # 最后一个月减持
cb2 = G.cb_gold_metrics(_d, _w2, [34000.0] * 20, 4400.0)
